          ...
          files.map((file) => {
            ...
            return (
              <div className="w-full" key={file.id}>
                <ul>
                  {((file.path.split('/')[5]).split('.')[1]==='jpg'  || (file.path.split('/')[5]).split('.')[1]==='png' || (file.path.split('/')[5]).split('.')[1]==='jpeg') &&
                  <li>
                  <span> <img width="60" height="60"src={file.path.split('./public')[1]}/> </span>
                  <button
